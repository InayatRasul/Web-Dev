...
def show_companies(request):
    if request.method == 'GET':
        ...
        serializer = CompanySerializer2(data = new_company)
...
def show_company(request, ind):
    ...
    if request.method == 'GET':
        ...
        output = CompanySerializer2(instance=company, data = dta)
        if output.is_valid():
            output.save()
            return JsonResponse(output.data)
...
def show_vacancies(request):
    ...
    if request.method == 'GET':
        ...
        new_vacancy = VacancySerializer2(data = my_data)
...
def show_vacancy(request, ind):
    ...
    if request.method == 'GET':
        ...
        my_data = json.loads(request.body)
        new_data = VacancySerializer2(vacancy, my_data)
